    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                Regex order = new Regex(@"@(?<planet>[A-Z][a-z]+)([^@\-!:>]*):(?<population>[\d]+)([^@\-!:>]*)!(?<attackType>[AD])![^@\-!:>]*->(?<soldierCount>\d+)*");
                                        //@(?<planet>[A-Z][a-z]+)([^@\-!:>]*):(?<population>[\d]+)([^@\-!:>]*)!(?<attackType>[AD])![^@\-!:>]*->(?<soldierCount>\d+)*
